 ...
 */
export function createRandomPoints(west, south, east, north, count, minI = 0.3, maxI = 1.0) {
    const points = [];
 ...
 */
export function createShareableUrl(location) {
 ...
 */
export function getDirectionFromCenter(centerLng, centerLat, lng, lat) {
    let direction = '';
    if (lat > centerLat) {
        direction += 'Norte';
    }
 ...
 */
export function groupNearbyPoints(points, maxDistanceKm) {
    const groups = [];
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        for (let j = 0; j < points.length; j++) {
            if (visited.has(j))
                continue;
            const distance = calculateDistance(points[i].latitude, points[i].longitude, points[j].latitude, points[j].longitude);
 ...
 */
export function findCentralPoint(points) {
    if (points.length === 0) {
