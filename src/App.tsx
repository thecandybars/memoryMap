...
mapboxgl.accessToken =
  "pk.eyJ1IjoianVhbmVsbzY2Nzk2IiwiYSI6ImNtNzNmZ2pzbTA4bjIyaXEzZ2F3a2dnOHcifQ.Ew2kIfF9F-Q1ltBQ0xOf-g";
...
const InfoButtons: React.FC<{
  ...
  return (
    <div className="fixed bottom-8 left-8 z-40 flex flex-col items-end gap-4">
      ...
      <div
        className="bg-black/50 backdrop-blur-md rounded-full p-1 
      ...
      >
        ...
        <button
          ...
          title={
            isLocationInfoVisible ? "Ocultar ubicación" : "Mostrar ubicación"
          }
        >
          <Info
            size={20}
            className={isLocationInfoVisible ? "text-white" : "text-white/80"}
          />
        </button>
...
export default function App() {
  ...
  const [highlightSection, setHighlightSection] = useState<
    "center" | "macro" | "department" | "memory"
  >("center");
    ...
    async function loadCSVData() {
      try {
        ...
        if (loadedData.length > 0) {
          console.log(
            `🟢 Datos REALES cargados exitosamente: ${loadedData.length} ubicaciones`
          );
          ...
          const memoryLocationsArray = await import("./utils/mapHelpers").then(
            (mod) => mod.memoryLocations
          );
  ...
  useEffect(() => {
    ...
    appState.selectedMemoryType,
    appState.selectedMacroRegion,
  ...
  useEffect(() => {
    ...
    if (appState.selectedMacroRegion) {
      ...
      if (appState.selectedDepartment) {
        const department = colombiaRegions[
          appState.selectedMacroRegion
        ].departments.find((d) => d.id === appState.selectedDepartment);
        if (department) {
  ...
  const handleDemoComplete = () => {
    try {
      console.log(
        "Completando modo demo y mostrando app principal con menú radial"
      );
  ...
  const handleRadialSelect = (
    ...
    console.log(
      `RadialSelect: tipo=${type}, id=${id}, stage=${appState.stage}`
    );
    ...
    if (type === "macro") {
      if (id === "center") {
        ...
        if (appState.stage === "app") {
          ...
          console.log(
            `Navegación ignorada, modo demo activo: ${appState.stage}`
          );
        }
      }
      ...
      setAppState((prev) => ({
        ...prev,
        selectedMemoryType:
          prev.selectedMemoryType === (id as keyof typeof memoryTypes)
            ? null
            : (id as keyof typeof memoryTypes),
      }));
      ...
      updateVisibleMarkersWithClustering(
        ...
        mapLoadedRef,
        {
          ...appState,
          selectedMemoryType:
            appState.selectedMemoryType === (id as keyof typeof memoryTypes)
              ? null
              : (id as keyof typeof memoryTypes),
        },
        handleMarkerClick
    ...
    <div className="absolute inset-0 overflow-hidden">
        ...
        {isMapLoaded && !isMapFullyLoaded && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999]">
            <div className="text-white text-center">
              <div className="mb-4">
                Cargando mapa ({mapProgress.toFixed(0)}%)
              </div>
      ...
      {appState.stage === "demo" && (
        <DemoMode
          onDemoComplete={() => {
            console.log(
              "DemoMode solicitó completar demo - llamando a handleDemoComplete"
            );
            handleDemoComplete();
          }}
      ...
      {(appState.stage === "app" || appState.stage === "tour") &&
        isMapFullyLoaded && (
          <>
              ...
              <>
                ...
                {!isMobileRef.current && (
                  <div
                    ...
                    className="fixed bottom-2 right-2 w-[400px] h-[400px] z-[9999] pointer-events-auto"
                  >
                    <RadialMenu
                ...
                {Object.keys(currentLocation).length > 0 && (
                  <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40 flex flex-col items-center">
                    <LocationContext
                      ...
                      onToggleVisibility={() =>
                        setIsLocationInfoVisible(!isLocationInfoVisible)
                      }
                    />
                  </div>
                )}
            ...
            {showLocationInfo && appState.selectedLocation && (
              <LocationInfo
                ...
                description={
                  appState.selectedLocation.description ||
                  "Este lugar forma parte de la red de sitios documentados por el Centro Nacional de Memoria Histórica como espacios significativos en la construcción de la memoria colectiva."
                }
                code={appState.selectedLocation.code}
                region={appState.selectedLocation.region}
                type={
                  appState.selectedLocation.type
                    ? memoryTypes[appState.selectedLocation.type]?.name
                    : "Lugar de Memoria"
                }
                onClose={() => {
